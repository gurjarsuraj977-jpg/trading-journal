...
let marketChartResizeHandlerBound=false;
...
async function loadMarketCandles(){
  ...
  const candles=(d.candles||[])
    .map(x=>({
      time:Math.floor(
        new Date(x.candle_time).getTime()/1000
      ),
      open:Number(x.open),
      high:Number(x.high),
      low:Number(x.low),
  ...
  if(!marketChartInstance){
    ...
    if(typeof LightweightCharts==="undefined"){
      throw Error(
        "Market chart library is not loaded."
      );
    }
    ...
    if(!marketChartResizeHandlerBound){

      marketChartResizeHandlerBound=true;
